...
VALUES (1, 'Glasses', '2024-11-15 08:00:00'),
...
VALUES ('Round, Black', '2024-11-15 08:30:00'),
...
SELECT ROUND(COUNT(DISTINCT h.user_id)::NUMERIC / COUNT(DISTINCT q.user_id), 2) AS quiz_to_home_try_on_rate
FROM quiz q
         LEFT JOIN home_try_on h ON q.user_id = h.user_id;

-- Home Try-On → Purchase conversion rate
SELECT ROUND(COUNT(DISTINCT p.user_id)::NUMERIC / COUNT(DISTINCT h.user_id), 2) AS home_try_on_to_purchase_rate
FROM home_try_on h
