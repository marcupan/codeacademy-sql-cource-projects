...
VALUES (1, '2024-11-15 08:00:00', 'Campaign A', 'Facebook', '1 - landing'),
...
WITH first_touch AS (SELECT user_id, MIN(timestamp) AS first_touch_time
                     FROM attribution_data
                     GROUP BY user_id)
...
WITH last_touch AS (SELECT user_id, MAX(timestamp) AS last_touch_time
                    FROM attribution_data
                    GROUP BY user_id)
